...
***/
	...
	while(L->First != NULL)
	{
		nextElem = L->First->ptr; // ukazatel na dalsi prvok v zozname
		free(L->First);
		L->First = nextElem; // presmerovanie ukazatela na prvy prvok
	}
}
...
** volá funkci Error().
**/
	tElemPtr newElem = malloc(sizeof(struct tElem)); // pomocny ukazatel na novo vytvoreny prvok
	if (newElem != NULL) 
	{
		newElem->data = val;
		newElem->ptr = L->First;
...
**/
	...
	{
		tElemPtr nextElem = L->Act->ptr->ptr; // pomocny ukazatel na dalsi prvok po mazanom prvku
		free(L->Act->ptr);
		L->Act->ptr = nextElem;	
	}
}
...
**/
	...
	{
		newElem->data = val;
		newElem->ptr = L->Act->ptr; // novy prvok ukazuje na dalsi prvok v zozname
		L->Act->ptr = newElem; // presmerovanie ukazatela na novy prvok
	}
}
