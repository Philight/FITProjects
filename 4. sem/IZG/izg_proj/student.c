...
S_Renderer * studrenCreate()
{
	...
	renderer->texture = loadBitmap(TEXTURE_FILENAME, &(renderer->tWidth), &(renderer->tHeight));
	...
	for (y = miny; y <= maxy; ++y)
	{
		...
		for (x = minx; x <= maxx; ++x)
		{
			if (e1 >= 0 && e2 >= 0 && e3 >= 0)
			{
				...
				double tx = (w1 * t[0].x / h1 + w2 * t[1].x / h2 + w3 * t[2].x / h3) / 
							(w1 * 1		 / h1 + w2 * 1		/ h2 + w3 * 1	   / h3);

				double ty = (w1 * t[0].y / h1 + w2 * t[1].y / h2 + w3 * t[2].y / h3) /
							(w1 * 1		 / h1 + w2 * 1		/ h2 + w3 * 1	   / h3);

				S_RGBA tColor = studrenTextureValue((S_StudentRenderer *)pRenderer, tx, ty);
				/*double tRed = tColor.red / 255;
				double tGreen = tColor.green / 255;
				double tBlue = tColor.blue / 255;*/
				/* interpolace barvy */
				color.red = ROUND2BYTE((w1 * col1.red + w2 * col2.red + w3 * col3.red) * tColor.red / 255);
				color.green = ROUND2BYTE((w1 * col1.green + w2 * col2.green + w3 * col3.green) * tColor.green / 255);
				color.blue = ROUND2BYTE((w1 * col1.blue + w2 * col2.blue + w3 * col3.blue) * tColor.blue / 255);
				color.alpha = 255;
...
void renderStudentScene(S_Renderer *pRenderer, S_Model *pModel)
{
	...
	S_Material MAT_WHITE_DIFFUSE = { 1.0, 1.0, 1.0, 1.0 };
